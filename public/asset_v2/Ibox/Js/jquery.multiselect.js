...
(function($){
    ...
    MultiSelect.prototype = {
        ...
        load: function() {
            ...
            optionsWrap.on('click', '.ms-selectall', function( event ){
                ...
                if( $(this).hasClass('global') ) {
                    ...
                    if( optionsList.find('li:not(.optgroup, .selected, .ms-hidden) input[type="checkbox"]:not(:disabled)').length ) {
                        ...
                        optionsList.find('.optgroup-all').prop( 'checked', true );
                    }
                    // deselect everything
                    else {
                        ...
                        optionsList.find('.optgroup-all').prop( 'checked', false );
                    }
                }
            ...
            optionsWrap.on('change', '.optgroup-all', function(event) {
                instance.updateSelectAll   = false;
                instance.updatePlaceholder = false;

                var select = optionsWrap.parent().siblings('.ms-list-' + instance.listNumber + '.jqmsLoaded');
                var isChecked = $(this).prop('checked'); // Check if checkbox is selected

                if ($(this).hasClass('global')) {
                    // Select or deselect all items globally
                    if (isChecked) {
                        // Select all unselected items
                        optionsList.find('li:not(.optgroup, .selected, .ms-hidden) input[type="checkbox"]:not(:disabled)')
                            .closest('li').addClass('selected')
                            .find('input[type="checkbox"]').prop('checked', true);
                    } else {
                        // Deselect all items
                        optionsList.find('li:not(.optgroup, .ms-hidden).selected input[type="checkbox"]:not(:disabled)')
                    ...
                    // Select or deselect items within the optgroup
                    if (isChecked) {
                        optgroup.find('li:not(.selected, .ms-hidden) input[type="checkbox"]:not(:disabled)')
                            .closest('li').addClass('selected')
                            .find('input[type="checkbox"]').prop('checked', true);
                    } else {
                        optgroup.find('li:not(.ms-hidden).selected input[type="checkbox"]:not(:disabled)')
                            .closest('li').removeClass('selected')
                            .find('input[type="checkbox"]').prop('checked', false);
                    }
                }
            ...
            for( var key in options ) {
                ...
                // OPTGROUP
                    ...
                    if( appendContainer ) {
                        ...
                        container.append('<input type="checkbox"  class="optgroup-all" value="'+ thisOption.label +'"> <a href="#" class="label">'+ thisOption.label +'</a>');
