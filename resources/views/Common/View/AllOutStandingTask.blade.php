        ...
        @if($task['task_category'] != 0 && $task['task_category'] == $edit_category)
            ...
            <td class="pivoted">
                ...
                <div class="d-flex justify-content-end  {{ PermissionDeniedDiv($permision) }}">
                    ...
                    <button class="btn btn-confirm me-1 {{ DisabledButtonOnRolePermission($permision) }}" patient-task-id="{{ $task['task_category'] }}" patient-id="{{ $task['patient_id'] }}" id="ibox_boardround_task_complete">Confirm</button>
