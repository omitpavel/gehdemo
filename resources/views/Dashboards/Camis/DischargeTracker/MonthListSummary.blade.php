...
@section('page-title', 'Month List Summary')
@section('page-top-title', 'Month List Summary')

@section('header')
    @parent
     <link rel="stylesheet" href="{{ asset('asset_v2/Template/Css/ResponsiveTable.css') }}" crossorigin="anonymous">
     <link rel="stylesheet" href="{{ asset('asset_v2/Template/Css/MonthListSummary.css') }}" crossorigin="anonymous">
@endsection
...
@section('footer')
    ...
    <script>
        function DataPageLoad(month){
            @if(CheckSpecificPermission('discharge_tracker_month_list_summary_view'))
                var token               = "{{ csrf_token() }}";
                $('.page-data-loader').show();
                setTimeout(function() {
                    $.ajax({
                        _token: token,
                        url: "{{ route('discharged.monthlist.data.load') }}",
                        type: 'POST',
