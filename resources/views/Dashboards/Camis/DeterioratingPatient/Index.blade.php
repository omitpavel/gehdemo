...
@section('page-title', 'Deteriorating New Patients')
@section('page-top-title', 'Deteriorating New Patients')
...
@push('custom-style')
    ...
    <link rel="stylesheet" href="{{ asset('asset_v2/Template/Css/ManageTask.css') }}" crossorigin="anonymous">
...
@section('footer')
    ...
    <script>
        var ajax_refresh_url = "{{ route('new.patient.dataload') }}";
        ...
        function DataPageLoad(ward_id, sort_by){
            @if(CheckSpecificPermission('dp_dashboard_new_patients_view'))
                ...
                setTimeout(function() {
                    $.ajax({
                        _token: token,
                        url: "{{ route('new.patient.dataload') }}",
                        type: 'GET',
        ...
        $(document).on("click", ".action_stepdown_patient", function(e) {
            @if(CheckSpecificPermission('dp_dashboard_patient_stepdown_action_update'))
                var token = "{{ csrf_token() }}";
                var camis_patient_id = $(this).data('patient-id');
                DisableButtonClickForPreventFurtherEvent("action_remove_" + camis_patient_id);
                if (camis_patient_id != '') {
                    var url = "{{ route('dp.action.stepdown') }}";
                    $.ajax({
                        url: url,
                        type: 'POST',
                        data: {
