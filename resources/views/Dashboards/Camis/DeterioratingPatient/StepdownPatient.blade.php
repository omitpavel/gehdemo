...
@section('page-title', 'Deteriorating  Patients Step Down')
@section('page-top-title', 'Deteriorating  Patients Step Down')
...
@section('footer')
    ...
    <script>
        var ajax_refresh_url = "{{ route('stepdown.patient.dataload') }}";
        function DataPageLoad(ward_id, sort_by){
            @if(CheckSpecificPermission('dp_dashboard_new_patients_view'))
                ...
                setTimeout(function() {
                    $.ajax({
                        _token: token,
                        url: "{{ route('stepdown.patient.dataload') }}",
                        type: 'GET',
