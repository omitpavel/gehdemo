...
@section('page-title', 'Deteriorating Patients Removed')
@section('page-top-title', 'Deteriorating Patients Removed')
...
@section('footer')
    ...
    <script>
        var ajax_refresh_url = "{{ route('removed.patient.dataload') }}";
        function CommonJsFunctionCallAfterContentRefersh(){
            MultiSelectJs('ward_id', 'Ward');
        }
        function DataPageLoad(ward_id, sort_by){
            @if(CheckSpecificPermission('dp_dashboard_removed_patients_view'))
                var token               = "{{ csrf_token() }}";
                $('.page-data-loader').show();
                setTimeout(function() {
                    $.ajax({
                        _token: token,
                        url: "{{ route('removed.patient.dataload') }}",
                        type: 'GET',
