                            ...
                            <div class="row mb-2">
                                <div class="col-lg-8 col-md-8 pe-md-1 mb-2 mb-md-0">
                                    <div class="mb-2 month-select">
                                        <select class="form-select w-100 breach_validation_select_heigh_set" aria-label="Default select example" name="date_picker_tab_5" id="date_picker_tab_5">
                                    ...
                                    <div class="@if ($loop->first) col pe-lg-0 border-breach-end @elseif($loop->last) col ps-lg-0 border-breach-end @else col ps-lg-0 pe-lg-0 border-breach-end @endif ">
                                        <div class="">
                                            ...
                                            <div class="breach-weekly-chart{{ $loop->iteration }}">
                                                <div id="gehBreachChartWeek{{ $loop->iteration }}"></div>
                                            </div>
                                        ...
                                        <div class="row">
                                            <div class="past-seven-days-section">
                                                <table class="table-breach">
                                                    ...
                                                    <tbody>
                                                        @if (count($success_array['past_seven_weeks']) > 0)
                                                            @foreach ($success_array['past_seven_weeks'] as $row)
                                                                <tr class="@if ($loop->even) bg-white @else bg-grey @endif">
                                                                    <td>{{ $row['date_val'] }}</td>
...
@foreach ($success_array['performance_by_speciality'] as $key => $row)
    <script>
        ...
        if (document.querySelector("#gehBreachChartWeek{{ $loop->iteration }}")) {
            if (chartweek{{ $loop->iteration }}) {
                chartweek{{ $loop->iteration }}.destroy();
            }
            var chartweek{{ $loop->iteration }} = new ApexCharts(document.querySelector("#gehBreachChartWeek{{ $loop->iteration }}"), options);
            chartweek{{ $loop->iteration }}.render();
        }
    </script>
@endforeach
